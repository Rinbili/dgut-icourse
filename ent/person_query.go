...
func (pq *PersonQuery) QueryObject() *ObjectQuery {
	query := (&ObjectClient{config: pq.config}).Query()
...
func (pq *PersonQuery) Clone() *PersonQuery {
	...
	return &PersonQuery{
		...
		withObject: pq.withObject.Clone(),
...
func (pq *PersonQuery) WithObject(opts ...func(*ObjectQuery)) *PersonQuery {
	query := (&ObjectClient{config: pq.config}).Query()
...
func (pq *PersonQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Person, error) {
	...
	if query := pq.withObject; query != nil {
		if err := pq.loadObject(ctx, query, nodes, nil,
			func(n *Person, e *Object) { n.Edges.Object = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PersonQuery) loadObject(ctx context.Context, query *ObjectQuery, nodes []*Person, init func(*Person), assign func(*Person, *Object)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Person)
	for i := range nodes {
		if nodes[i].object_person == nil {
			continue
		}
		fk := *nodes[i].object_person
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "object_person" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
