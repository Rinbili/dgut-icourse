...
func (pc *PersonCreate) SetObjectID(id uuid.UUID) *PersonCreate {
	pc.mutation.SetObjectID(id)
	return pc
}

// SetObject sets the "object" edge to the Object entity.
func (pc *PersonCreate) SetObject(o *Object) *PersonCreate {
	return pc.SetObjectID(o.ID)
}
...
func (pc *PersonCreate) check() error {
	if _, ok := pc.mutation.ObjectID(); !ok {
		return &ValidationError{Name: "object", err: errors.New(`ent: missing required edge "Person.object"`)}
	}
	return nil
}
...
func (pc *PersonCreate) createSpec() (*Person, *sqlgraph.CreateSpec) {
	...
	if nodes := pc.mutation.ObjectIDs(); len(nodes) > 0 {
