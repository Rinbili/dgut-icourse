...
func (oq *OrganizationQuery) QueryObject() *ObjectQuery {
...
func (oq *OrganizationQuery) WithObject(opts ...func(*ObjectQuery)) *OrganizationQuery {
	query := (&ObjectClient{config: oq.config}).Query()
...
func (oq *OrganizationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Organization, error) {
	...
	if query := oq.withObject; query != nil {
		if err := oq.loadObject(ctx, query, nodes, nil,
			func(n *Organization, e *Object) { n.Edges.Object = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oq *OrganizationQuery) loadObject(ctx context.Context, query *ObjectQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Object)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Organization)
	for i := range nodes {
		if nodes[i].object_organization == nil {
			continue
		}
		fk := *nodes[i].object_organization
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "object_organization" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
