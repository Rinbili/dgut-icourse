...
func (oq *OtherQuery) QueryObject() *ObjectQuery {
	query := (&ObjectClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(other.Table, other.FieldID, selector),
...
func (oq *OtherQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (oq *OtherQuery) Clone() *OtherQuery {
	...
	return &OtherQuery{
		...
		withObject: oq.withObject.Clone(),
...
func (oq *OtherQuery) WithObject(opts ...func(*ObjectQuery)) *OtherQuery {
	query := (&ObjectClient{config: oq.config}).Query()
...
func (oq *OtherQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Other, error) {
	...
	if oq.withObject != nil {
		withFKs = true
	}
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, other.ForeignKeys...)
	}
	...
	if query := oq.withObject; query != nil {
		if err := oq.loadObject(ctx, query, nodes, nil,
			func(n *Other, e *Object) { n.Edges.Object = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oq *OtherQuery) loadObject(ctx context.Context, query *ObjectQuery, nodes []*Other, init func(*Other), assign func(*Other, *Object)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Other)
	for i := range nodes {
		if nodes[i].object_other == nil {
			continue
		}
		fk := *nodes[i].object_other
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "object_other" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (oq *OtherQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(other.Table, other.Columns, sqlgraph.NewFieldSpec(other.FieldID, field.TypeInt))
	_spec.From = oq.sql
