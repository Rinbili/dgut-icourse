...
func (e PersonEdges) ObjectOrErr() (*Object, error) {
	if e.Object != nil {
		return e.Object, nil
...
func (*Person) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case person.ForeignKeys[0]: // object_person
	...
	for i := range columns {
		...
		case person.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				return fmt.Errorf("unexpected type %T for field object_person", values[i])
			} else if value.Valid {
				pe.object_person = new(uuid.UUID)
				*pe.object_person = *value.S.(*uuid.UUID)
			}
		default:
...
// QueryObject queries the "object" edge of the Person entity.
func (pe *Person) QueryObject() *ObjectQuery {
	return NewPersonClient(pe.config).QueryObject(pe)
}
