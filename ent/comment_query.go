...
func (cq *CommentQuery) QueryObject() *ObjectQuery {
...
// QueryCourseComment chains the current query on the "course_comment" edge.
func (cq *CommentQuery) QueryCourseComment() *CourseCommentQuery {
	query := (&CourseCommentClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CommentQuery) WithObject(opts ...func(*ObjectQuery)) *CommentQuery {
	query := (&ObjectClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithCourseComment tells the query-builder to eager-load the nodes that are connected to
// the "course_comment" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CommentQuery) WithCourseComment(opts ...func(*CourseCommentQuery)) *CommentQuery {
	query := (&CourseCommentClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if query := cq.withObject; query != nil {
		if err := cq.loadObject(ctx, query, nodes, nil,
			func(n *Comment, e *Object) { n.Edges.Object = e }); err != nil {
			return nil, err
		}
	}
...
func (cq *CommentQuery) loadObject(ctx context.Context, query *ObjectQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Object)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Comment)
	for i := range nodes {
		if nodes[i].object_comments == nil {
			continue
		}
		fk := *nodes[i].object_comments
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "object_comments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (cq *CommentQuery) loadCourseComment(ctx context.Context, query *CourseCommentQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *CourseComment)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Comment)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.CourseComment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(comment.CourseCommentColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.comment_course_comment
		if fk == nil {
			return fmt.Errorf(`foreign-key "comment_course_comment" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
