...
func (uq *UserQuery) QueryInfo() *UserInfoQuery {
...
func (uq *UserQuery) WithInfo(opts ...func(*UserInfoQuery)) *UserQuery {
	query := (&UserInfoClient{config: uq.config}).Query()
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withInfo; query != nil {
		if err := uq.loadInfo(ctx, query, nodes, nil,
			func(n *User, e *UserInfo) { n.Edges.Info = e }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadInfo(ctx context.Context, query *UserInfoQuery, nodes []*User, init func(*User), assign func(*User, *UserInfo)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*User)
	for i := range nodes {
		if nodes[i].user_info == nil {
			continue
		}
		fk := *nodes[i].user_info
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_info" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (uq *UserQuery) loadComments(ctx context.Context, query *CommentQuery, nodes []*User, init func(*User), assign func(*User, *Comment)) error {
	...
	for _, n := range neighbors {
		fk := n.comment_author
		if fk == nil {
			return fmt.Errorf(`foreign-key "comment_author" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "comment_author" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
