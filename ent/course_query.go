...
type CourseQuery struct {
	...
	withTeacher      *ObjectQuery
	withOrganization *ObjectQuery
	withFKs          bool
...
func (cq *CourseQuery) QueryObject() *ObjectQuery {
...
func (cq *CourseQuery) QueryTeacher() *ObjectQuery {
	query := (&ObjectClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (cq *CourseQuery) QueryOrganization() *ObjectQuery {
	query := (&ObjectClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (cq *CourseQuery) Clone() *CourseQuery {
	...
	return &CourseQuery{
		...
		withObject:       cq.withObject.Clone(),
...
func (cq *CourseQuery) WithObject(opts ...func(*ObjectQuery)) *CourseQuery {
	query := (&ObjectClient{config: cq.config}).Query()
...
func (cq *CourseQuery) WithTeacher(opts ...func(*ObjectQuery)) *CourseQuery {
	query := (&ObjectClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CourseQuery) WithOrganization(opts ...func(*ObjectQuery)) *CourseQuery {
	query := (&ObjectClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CourseQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Course, error) {
	...
	if cq.withObject != nil || cq.withTeacher != nil || cq.withOrganization != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := cq.withObject; query != nil {
		if err := cq.loadObject(ctx, query, nodes, nil,
			func(n *Course, e *Object) { n.Edges.Object = e }); err != nil {
			return nil, err
		}
	}
	if query := cq.withTeacher; query != nil {
		if err := cq.loadTeacher(ctx, query, nodes, nil,
			func(n *Course, e *Object) { n.Edges.Teacher = e }); err != nil {
			return nil, err
		}
	}
	if query := cq.withOrganization; query != nil {
		if err := cq.loadOrganization(ctx, query, nodes, nil,
			func(n *Course, e *Object) { n.Edges.Organization = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CourseQuery) loadObject(ctx context.Context, query *ObjectQuery, nodes []*Course, init func(*Course), assign func(*Course, *Object)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Course)
	for i := range nodes {
		if nodes[i].object_course == nil {
			continue
		}
		fk := *nodes[i].object_course
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "object_course" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (cq *CourseQuery) loadTeacher(ctx context.Context, query *ObjectQuery, nodes []*Course, init func(*Course), assign func(*Course, *Object)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Course)
	for i := range nodes {
		if nodes[i].object_teach_courses == nil {
			continue
		}
		fk := *nodes[i].object_teach_courses
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "object_teach_courses" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (cq *CourseQuery) loadOrganization(ctx context.Context, query *ObjectQuery, nodes []*Course, init func(*Course), assign func(*Course, *Object)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Course)
	for i := range nodes {
		if nodes[i].object_setup_courses == nil {
			continue
		}
		fk := *nodes[i].object_setup_courses
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "object_setup_courses" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
